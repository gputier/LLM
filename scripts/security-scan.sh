#!/usr/bin/env bash
# Trivy scan of the whole tree: plaintext secrets and misconfigurations.
#
# This repository holds three submodules and a README, so a push from here only
# moves submodule pointers. The scan still walks the checked out submodules,
# which is the point: it is the one place that sees all three stations at once,
# and a secret pasted into any of them is caught here even if that station's own
# gate was never wired in this clone.
#
# Blocking policy, identical to the other repositories of the house:
...
#
# Fails closed. Exit code 2 is reserved for "findings", so every other non-zero
# code means the scan itself broke: trivy fs --exit-code 2 returns 2 on findings
# and 1 on a real error, a single --exit-code 1 cannot tell the two apart.
#
...
# of 1.16 s on a station repository, measured on 2026-09-18, and gives up the
# one place where a remote URL carrying a token is written in plaintext,
...
removed from the file, and the history is checked for earlier copies. If it sits
in a submodule, fix it there and push that repository first.
